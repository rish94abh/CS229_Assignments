...
class QP:
    ...
    def gradient(self, X, Y):
        ...
        #grad_theta = (2 / m) * X.T.dot(error * self.theta)
        #grad_phi = (-2 / m) * X.T.dot(error * self.phi)

        grad_theta = (2 / m) * (X * self.theta).T.dot(error).flatten()
        grad_phi = (-2 / m) * (X * self.phi).T.dot(error).flatten()
        
        # Gradient clipping to prevent overflow
        grad_theta = np.clip(grad_theta, -1e20, 1e20)
        grad_phi = np.clip(grad_phi, -1e20, 1e20)
