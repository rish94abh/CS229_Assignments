...
def regression(train_path, validation_path):
    ...
    val_err = np.mean((y_pred - y_validation) ** 2)
    # *** END CODE HERE
    return val_err
...
def ridge_regression(train_path, validation_path):
    ...
    # *** START CODE HERE ***
    for reg in reg_list:
        # Compute beta_hat using the ridge regression normal equation
        reg_matrix = reg * np.eye(x_train.shape[1])
        beta_hat = np.linalg.inv(x_train.T @ x_train + reg_matrix) @ x_train.T @ y_train

        # Compute validation error
        y_pred = x_validation @ beta_hat
        val_err.append(np.mean((y_pred - y_validation) ** 2))
    # *** END CODE HERE
    return val_err
